...
- `部署 GitHub Pages`：人工推送到 `main`，或每日抓取、讲座/来源审核、Owner 设置成功完成后，自动读取最新 `main` 并部署。
...
本地采集器使用 Docker Desktop 与私有 WeRSS。电脑无需全天在线：WeRSS 每天 08:17 抓取一次；错过计划时会在服务恢复后继续处理，授权失效时需重新扫码。

本机桥接位于 `local_bridge/`：读取 `data/sources.json` 中已启用且配置了 `feedId` 的公众号，只把公众号名、标题、发布日期和微信原文链接提交到 GitHub；跨次去重，每天最多 10 条。`crawlPriority` 只控制读取次序，不绕过内容筛选；当前优先处理 `PKU学生创新学社` 和 `P-Lib official`。Token 由 `install.ps1` 交互式读取，经 Windows 当前用户加密后保存在本机，不进入仓库或日志。桥接任务每天 08:27 运行一次。
